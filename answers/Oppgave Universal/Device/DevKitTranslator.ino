...
// Language
static int currentToLanguage = 1;
static int currentFromLanguage = 1;
static const char *allLanguages[LANGUAGES_COUNT] = {"Norsk", "English", "French", "German", "Italian", "Portuguese", "Spanish"};
static const char *allLocalNames[LANGUAGES_COUNT] = {"nb-NO", "en-GB", "fr-FR", "de-DE", "it-IT", "pt-BR", "es-ES"};
...
static void EnterSelectToLanguageState(bool clean = true)
{
  status = SelectToLanguage;
  Screen.clean();
  Screen.print(0, "Press B Scroll");
  ShowLanguages(currentToLanguage);
}
...
static int HttpTriggerTranslator(const char *content, int length)
{
  ...
  sprintf(azureFunctionUri, "%s&&source=%s&target=%s", (char *)AZURE_FUNCTION_URL, allLocalNames[currentFromLanguage], allLocalNames[currentToLanguage]);
  HTTPClient client = HTTPClient(HTTP_POST, azureFunctionUri);
  client.set_header("source", allLocalNames[currentFromLanguage]);
  client.set_header("target", allLocalNames[currentToLanguage]);
...
static void ShowLanguages(int currentLanguage)
{
  char temp[20];
...
static void DoIdle()
{
  if (digitalRead(USER_BUTTON_A) == LOW)
  {
    ...
    status = SelectFromLanguage;
    Screen.clean();
    Screen.print(0, "Press B Scroll");
    ShowLanguages(currentFromLanguage);
  }
...
static void DoSelectToLanguage()
{
  if (digitalRead(USER_BUTTON_B) == LOW)
  {
    currentToLanguage = (currentToLanguage + 1) % LANGUAGES_COUNT;
    ShowLanguages(currentToLanguage);
  }
...
static void DoSelectFromLanguage()
{
  if (digitalRead(USER_BUTTON_B) == LOW)
  {
    currentFromLanguage = (currentFromLanguage + 1) % LANGUAGES_COUNT;
    ShowLanguages(currentFromLanguage);
  }
  else if (digitalRead(USER_BUTTON_A) == LOW)
  {
    EnterSelectToLanguageState();
  }
  DevKitMQTTClient_Check();
}
